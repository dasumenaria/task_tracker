    ...
    public function projectListbyUser($user_id=null) 
    {
		...
		if($user_id!=1){ 
			$count=$this->Projects->find()
				...
				}])->where(['Projects.completed_status IN'=>['0','2']])->count();
		...
		if($count > 0){
			...
			$response_object = $this->Projects->find()
				...
				}])->where(['Projects.completed_status IN'=>['0','2']]);
...
public function projectDetails($project_id=null,$user_id=null)
    {
		...
		if($user_id!=1){ 
			$count=$this->Projects->find()
				...
				->contain(['MasterClients'=>['MasterClientPocs'],'Users','ProjectMembers'])->contain(['Tasks'=>function($q) use($user_id){
					return $q->where(['Tasks.status IN'=>['0','2']])->order(['Tasks.deadline'=>'ASC'])->innerJoinWith('TaskMembers',function ($q) use($user_id){
					    return $q->where(['TaskMembers.user_id'=>$user_id])->contain(['Users']);
					});
				}])
				->where(['Projects.id'=>$project_id,'Projects.completed_status IN'=>['0','2']])->count();
			...
			$count=$this->Projects->find()
				->contain(['MasterClients'=>['MasterClientPocs'],'Users','ProjectMembers'=>
					...
					return $q->where(['Tasks.status IN'=>['0','2']])->order(['Tasks.deadline'=>'ASC'])->innerJoinWith('TaskMembers',function($q){
		...
		if($count>0){ 
			if($user_id == 1){
				$response_object = $this->Projects->get($project_id,[
					...
					'conditions' => ['Projects.completed_status IN'=>['0','2']],
				]);
			}else{
				$response_object = $this->Projects->find()
					->innerJoinWith('ProjectMembers',function($q)use($user_id){
				...
				->contain(['Tasks'=>function($q) use($user_id){
					return $q->where(['Tasks.status IN'=>['0','2']])->order(['Tasks.deadline'=>'ASC'])->innerJoinWith('TaskMembers',function ($q) use($user_id){
					    return $q->where(['TaskMembers.user_id'=>$user_id])->contain(['Users']);
					});
				}])
					->where(['Projects.id'=>$project_id,'Projects.completed_status IN'=>['0','2']])->first();
			}
			...
			foreach($tasks as $k=>$task){   
				foreach($task as $key=>$tasks_deadline){
		...
		//	$a= (object)$tasks;
		//	$response_object['tasks'] = (object)$a;
