...
//$user_ids = implode(' ',$user_ids);
        ...
        //$abc = json_encode($abc);
        $this->set(compact('success','error','abc'));
        $this->set('_serialize', ['success','error','abc']);
    }
	 
	 
	 
	 public function CreateTaskForIos()
    {	 
		$project_id=$this->request->getData('project_id');
		$save_type=$this->request->getData('save_type');
		$title=$this->request->getData('title');
		$login_id=$this->request->getData('login_id');
	    $user_ids=json_decode($this->request->getData('user_id'));
		$deadline=$this->request->getData('deadline');
		//$abc = $this->request->getData();
		//pr($this->request->getData());exit;
		 $task = $this->Tasks->newEntity();
...
//$user_ids = implode(' ',$user_ids);
	...
	public function CompletedList()
    {
		...
		if($count>0){
			if($user_id!=1){
				$Response = $this->Tasks->find()->where(['Tasks.status'=>1,'Tasks.project_id'=>$project_id])->contain(['Projects','TaskMembers'=>
				function ($q) use($user_id){
				    return $q->where(['TaskMembers.user_id'=>$user_id])->contain(['Users']);
				}])
				->order(['Tasks.completed_on'=>'DESC']);
			}
			else{
				$Response = $this->Tasks->find()->where(['Tasks.status'=>1,'Tasks.project_id'=>$project_id])->contain(['Projects','TaskMembers'=>['Users']])->order(['Tasks.completed_on'=>'DESC']);
			}
			$success=true;
			$error=''; 
		}
